...
#ifndef LEETCODE_OPTIMAL_SOLUTIONS_SOLUTION0205_同构字符串_H
#define LEETCODE_OPTIMAL_SOLUTIONS_SOLUTION0205_同构字符串_H
 ...
 * 用  _table 存下来各个字母出现的位置，然后根据该 _table 遍历字符串2 对应的字母对应位置是否相同，出错的时机，会更早一些。
 * 但是这需要做两个方向的映射，因为如果前者更细碎，那么后者一定会成功。
 *
 * 上述提及了两个方法
 * 1.编码法，类似哈夫曼编码，寻求一种编码方式，让类型对应的位置可以唯一对应一种编码
 * 2._table 双向遍历法，需要两个人都算出 _table 然后根据 _table 遍历对方。
 *
 * 最好的应该是 3.一次 _table 法，利用 char char _table，一次取出对应位置，然后在遍历 s 字符串的过程中判断 t 中相应位置是否始终是该值。草，可以这样的。
...
#endif //LEETCODE_OPTIMAL_SOLUTIONS_SOLUTION0205_同构字符串_H
